"""PeptideBuilder: fragment-based peptide design around a small-molecule ligand
using the FAIRChem UMA potential.

Usage:
    python peptide_builder.py list
    python peptide_builder.py run <built-in name | file.xyz | SMILES> [options]

Functions can also be imported in a notebook: `from peptide_builder import *`.
"""
import argparse
import copy
import csv
import os
import random
import sys
import time

import numpy as np
import ase.io
from ase import Atoms
from ase.constraints import FixAtoms
from ase.optimize import BFGS

octinoxate_data = {
    "num_atoms": 44,
    "name": "octinoxate",
    "atoms": ['C', 'C', 'C', 'C', 'C', 'C', 'H', 'H', 'H', 'H', 'O', 'C', 'H', 'H', 'H', 'C', 'H', 'C', 'H', 'C', 'O', 'O', 'C', 'H', 'C', 'H', 'H', 'C', 'H', 'H', 'C', 'H', 'H', 'H', 'C', 'H', 'H', 'C', 'H', 'H', 'C', 'H', 'H', 'H'],
    "coords": [
        [-5.945350681818182, 0.23278688636363637, 0.16006995454545458],
        [-5.172542681818181, 1.3434288863636366, 0.4925409545454545],
        [-3.7998686818181815, 1.2793748863636365, 0.39736195454545453],
        [-3.1473386818181814, 0.11847288636363638, -0.02927704545454544],
        [-3.9371006818181815, -0.9809231136363636, -0.35633604545454545],
        [-5.3173046818181815, -0.9356211136363636, -0.26665404545454546],
        [-5.670811681818182, 2.2442728863636363, 0.8226839545454546],
        [-3.2111576818181815, 2.1500078863636363, 0.6591099545454546],
        [-3.4746566818181814, -1.9003121136363634, -0.6895730454545455],
        [-5.890931681818182, -1.8116101136363636, -0.5298930454545454],
        [-7.280922681818182, 0.38014688636363636, 0.2814289545454545],
        [-8.110493681818182, -0.7119621136363636, -0.04155404545454544],
        [-7.905391681818181, -1.5729501136363635, 0.6002269545454546],
        [-9.130393681818182, -0.3767111136363636, 0.12538195454545453],
        [-7.995182681818181, -1.0053401136363636, -1.0883910454545456],
        [-1.6885196818181818, 0.1151798863636364, -0.11031904545454545],
        [-1.2043856818181817, 1.0389918863636365, 0.18673795454545455],
        [-0.9047716818181817, -0.8889731136363637, -0.5051070454545454],
        [-1.2957356818181818, -1.8473391136363635, -0.8184200454545454],
        [0.5661873181818183, -0.8176351136363637, -0.5564920454545454],
        [1.2535253181818184, -1.7482521136363636, -0.9030040454545454],
        [1.0536433181818183, 0.3733468863636364, -0.18911604545454547],
        [2.4877833181818185, 0.5414958863636363, -0.18817904545454545],
        [2.8849863181818183, -0.05701711363636361, -1.0092380454545455],
        [3.0103743181818183, 0.017014886363636388, 1.1451719545454544],
        [2.6290143181818184, -0.9973781136363636, 1.2700839545454545],
        [2.5647793181818184, 0.6213728863636364, 1.9394939545454546],
        [2.7346373181818184, 2.0235678863636366, -0.45721604545454547],
        [1.9231723181818183, 2.3720828863636365, -1.0974950454545456],
        [2.6544743181818182, 2.5825578863636367, 0.47880195454545454],
        [4.060457318181818, 2.3306668863636366, -1.1452480454545455],
        [4.128206318181818, 3.3932688863636367, -1.3785320454545456],
        [4.920967318181819, 2.0744828863636364, -0.5298220454545455],
        [4.147235318181818, 1.7791168863636364, -2.0829780454545452],
        [4.529157318181818, 0.002277886363636388, 1.3127209545454546],
        [4.910828318181818, 1.0251098863636365, 1.3471389545454544],
        [4.745812318181819, -0.4278371136363636, 2.2944199545454547],
        [5.292977318181818, -0.7928611136363636, 0.25283095454545457],
        [6.356103318181819, -0.7603821136363637, 0.5026069545454546],
        [5.199829318181818, -0.30302511363636364, -0.7195700454545454],
        [4.841835318181818, -2.2437721136363633, 0.12912195454545455],
        [5.478644318181819, -2.7908341136363637, -0.5665030454545454],
        [4.892125318181819, -2.7509921136363635, 1.0953669545454545],
        [3.816104318181818, -2.3172971136363634, -0.23438304545454547],
    ],
    "charge": 0,
    "spin": 1,
    "size": 0.0
}

octocrylene_data = {
    "num_atoms": 56,
    "name": "octocrylene",
    "atoms": ['C', 'C', 'C', 'C', 'C', 'C', 'H', 'H', 'H', 'H', 'H', 'C', 'H', 'C', 'C', 'C', 'C', 'H', 'C', 'H', 'C', 'H', 'H', 'H', 'C', 'H', 'C', 'O', 'C', 'N', 'O', 'C', 'H', 'H', 'C', 'H', 'C', 'H', 'H', 'C', 'H', 'H', 'H', 'C', 'H', 'H', 'C', 'H', 'H', 'C', 'H', 'H', 'C', 'H', 'H', 'H'],
    "coords": [
        [4.104155589285714, 0.007694517857142813, -1.5863354821428572],
        [5.336717589285714, 0.5401815178571427, -1.9294634821428573],
        [5.963086589285714, 1.444143517857143, -1.085195482142857],
        [5.349236589285714, 1.811554517857143, 0.10228751785714284],
        [4.115592589285715, 1.279299517857143, 0.44331951785714285],
        [3.4810805892857144, 0.3724145178571428, -0.39783548214285713],
        [3.6216165892857144, -0.7053074821428572, -2.243012482142857],
        [5.807968589285714, 0.2482135178571428, -2.858577482142857],
        [6.924160589285714, 1.862514517857143, -1.352839482142857],
        [5.829646589285714, 2.519072517857143, 0.7651115178571428],
        [3.6376195892857144, 1.583221517857143, 1.3660195178571428],
        [2.1541105892857146, -0.2683674821428572, -0.021720482142857156],
        [1.6484565892857144, -0.5481944821428573, -0.9468344821428571],
        [1.2335525892857144, 0.6706295178571429, 0.7331225178571429],
        [0.5853995892857145, 1.6760195178571429, 0.01958551785714284],
        [1.0058025892857145, 0.5796375178571429, 2.100790517857143],
        [-0.2705654107142855, 2.560606517857143, 0.6501995178571428],
        [0.7560275892857145, 1.7601175178571427, -1.0470014821428573],
        [0.14887458928571445, 1.4676995178571428, 2.738772517857143],
        [1.4879995892857145, -0.1910674821428572, 2.6877975178571427],
        [-0.4921184107142854, 2.459415517857143, 2.016877517857143],
        [-0.7720474107142855, 3.3272085178571427, 0.07435651785714284],
        [-0.017559410714285573, 1.3770775178571428, 3.803877517857143],
        [-1.1634814107142857, 3.147541517857143, 2.5126245178571427],
        [2.4236995892857145, -1.5618724821428571, 0.7159165178571428],
        [3.0348865892857146, -1.490914482142857, 1.6112405178571427],
        [1.5260015892857144, -2.7226394821428572, 0.5723825178571429],
        [2.7420085892857147, -2.697602482142857, -0.09256748214285715],
        [1.4281475892857145, -3.687209482142857, 1.673176517857143],
        [1.3152245892857146, -4.405578482142857, 2.5595505178571427],
        [0.4253365892857145, -2.678553482142857, -0.22939848214285719],
        [-0.7487774107142856, -2.1555644821428572, 0.4090415178571428],
        [-1.3652954107142854, -2.9926814821428573, 0.7463065178571429],
        [-0.4555934107142855, -1.579344482142857, 1.2886445178571428],
        [-1.4932554107142852, -1.2526334821428573, -0.5591214821428572],
        [-0.7448914107142856, -0.5688334821428572, -0.9694034821428572],
        [-2.0938674107142856, -2.0265244821428574, -1.7395864821428573],
        [-2.392763410714285, -1.3107414821428571, -2.508177482142857],
        [-1.3036654107142853, -2.6337034821428573, -2.185548482142857],
        [-3.2853164107142856, -2.912874482142857, -1.3948394821428571],
        [-3.039631410714285, -3.6468504821428573, -0.6258794821428572],
        [-4.132982410714286, -2.327521482142857, -1.0356094821428572],
        [-3.6172954107142856, -3.464791482142857, -2.274037482142857],
        [-2.4941164107142852, -0.3970304821428572, 0.21890751785714285],
        [-1.9525834107142854, 0.1111425178571428, 1.022233517857143],
        [-3.2402954107142854, -1.0324114821428572, 0.7063755178571428],
        [-3.1933744107142856, 0.6580625178571429, -0.6267984821428572],
        [-3.8309684107142856, 0.18316451785714277, -1.3781884821428572],
        [-2.4378694107142853, 1.2263075178571428, -1.1802564821428572],
        [-4.030917410714286, 1.620355517857143, 0.20542451785714283],
        [-3.3890224107142854, 2.086932517857143, 0.9581945178571428],
        [-4.786954410714286, 1.055215517857143, 0.7578315178571429],
        [-4.705796410714286, 2.697496517857143, -0.6323974821428572],
        [-5.372235410714286, 2.2562225178571427, -1.3759484821428571],
        [-5.2972994107142855, 3.3740205178571427, -0.014965482142857173],
        [-3.965870410714285, 3.295630517857143, -1.1684284821428572],
    ],
    "charge": 0,
    "spin": 1,
    "size": 0.0
}

oxybenzone_data = {
    "num_atoms": 29,
    "name": "oxybenzone",
    "atoms": ['C', 'C', 'C', 'C', 'C', 'C', 'H', 'C', 'O', 'C', 'C', 'C', 'C', 'H', 'C', 'H', 'C', 'H', 'H', 'H', 'H', 'O', 'C', 'H', 'H', 'H', 'O', 'H', 'H'],
    "coords": [
        [2.8035907241379308, -0.4830889310344828, 0.31171544827586206],
        [1.7467487241379311, -1.1601059310344828, 0.919205448275862],
        [0.4922377241379311, -0.5957459310344828, 0.9020494482758621],
        [0.23824372413793113, 0.6453840689655173, 0.3159804482758621],
        [1.3068487241379312, 1.3102510689655171, -0.2873885517241379],
        [2.5812027241379307, 0.7448800689655172, -0.2971815517241379],
        [3.385087724137931, 1.2793680689655171, -0.7846865517241379],
        [-1.1168162758620688, 1.2651360689655171, 0.4203244482758621],
        [-1.2606222758620689, 2.4213810689655175, 0.741730448275862],
        [-2.313361275862069, 0.40184506896551725, 0.1452914482758621],
        [-2.251433275862069, -0.7107779310344828, -0.6882195517241378],
        [-3.534515275862069, 0.7684500689655173, 0.705222448275862],
        [-3.394730275862069, -1.4473629310344829, -0.9568835517241381],
        [-1.3101642758620689, -0.9932989310344829, -1.140039551724138],
        [-4.671713275862069, 0.02369006896551723, 0.4503834482758621],
        [-3.5719842758620692, 1.6455220689655172, 1.336883448275862],
        [-4.603332275862068, -1.0852739310344828, -0.3828185517241379],
        [-3.341309275862069, -2.3041529310344826, -1.615228551724138],
        [-5.615068275862068, 0.30802206896551726, 0.8972854482758621],
        [-5.494159275862069, -1.6648629310344827, -0.5867205517241378],
        [1.933219724137931, -2.1131469310344824, 1.391999448275862],
        [4.004751724137932, -1.0934609310344827, 0.3572304482758621],
        [5.113187724137932, -0.45214493103448283, -0.22870755172413793],
        [5.963530724137931, -1.1085179310344828, -0.06605955172413791],
        [4.9706357241379315, -0.3100049310344828, -1.303546551724138],
        [5.310409724137932, 0.5144450689655172, 0.24278844827586207],
        [1.0733537241379312, 2.4912180689655172, -0.8947645517241378],
        [1.882667724137931, 2.8245570689655173, -1.2809085517241379],
        [-0.32650727586206885, -1.1222039310344827, 1.3750634482758621],
    ]
    ,
    "charge": 0,
    "spin": 1,
    "size": 0.0
}

avobenzone_data = {
    "num_atoms": 45,
    "name": "avobenzone",
    "atoms": ['C', 'C', 'C', 'C', 'C', 'C', 'H', 'H', 'H', 'H', 'C', 'C', 'H', 'H', 'H', 'C', 'H', 'H', 'H', 'C', 'H', 'H', 'H', 'C', 'C', 'H', 'H', 'C', 'C', 'C', 'C', 'C', 'H', 'C', 'H', 'C', 'H', 'H', 'O', 'C', 'H', 'H', 'H', 'O', 'O'],
    "coords": [
        [-3.2471907111111107, -0.029338311111111015, 0.1320089111111111],
        [-2.515536711111111, 0.33976368888888897, -0.990419088888889],
        [-1.3286977111111107, 1.049899688888889, -0.8800080888888889],
        [-0.8368777111111108, 1.410781688888889, 0.3680959111111111],
        [-1.5619977111111107, 1.043048688888889, 1.5022439111111112],
        [-2.739998711111111, 0.34011868888888896, 1.3824579111111113],
        [-2.8607047111111106, 0.07535068888888899, -1.9788270888888888],
        [-0.7871387111111108, 1.3001076888888892, -1.7816810888888888],
        [-1.1803057111111106, 1.3209526888888892, 2.475009911111111],
        [-3.2772937111111107, 0.07149568888888896, 2.282448911111111],
        [-4.557832711111111, -0.8110383111111111, 0.0471959111111111],
        [-5.68265771111111, 0.016693688888888975, 0.6883079111111111],
        [-6.626711711111111, -0.528413311111111, 0.6334819111111111],
        [-5.481231711111111, 0.22989268888888897, 1.737937911111111],
        [-5.807573711111111, 0.9683996888888889, 0.1699959111111111],
        [-4.403201711111111, -2.139125311111111, 0.8047019111111111],
        [-5.331255711111111, -2.711176311111111, 0.751051911111111],
        [-3.6046927111111104, -2.7422903111111108, 0.3705299111111111],
        [-4.169711711111111, -1.980302311111111, 1.8573259111111111],
        [-4.956199711111111, -1.125447311111111, -1.3962720888888889],
        [-5.106825711111111, -0.21725031111111104, -1.9816000888888887],
        [-4.207291711111111, -1.739359311111111, -1.8985260888888889],
        [-5.894987711111111, -1.680617311111111, -1.4009420888888888],
        [0.42969228888888933, 2.170386688888889, 0.5539479111111111],
        [1.2380222888888892, 2.575069688888889, -0.677167088888889],
        [0.5865622888888893, 3.029724688888889, -1.4205450888888889],
        [1.9648132888888894, 3.3129756888888893, -0.3437100888888889],
        [1.9480212888888893, 1.412276688888889, -1.3710050888888887],
        [3.120996288888889, 0.7908566888888889, -0.7090350888888889],
        [3.6197622888888894, 1.219004688888889, 0.5248689111111111],
        [3.7539692888888894, -0.270231311111111, -1.349901088888889],
        [4.71393328888889, 0.604695688888889, 1.0917649111111112],
        [3.143627288888889, 2.027687688888889, 1.0616639111111112],
        [4.853690288888889, -0.894795311111111, -0.793587088888889],
        [3.3656892888888894, -0.601821311111111, -2.303181088888889],
        [5.340944288888889, -0.455538311111111, 0.4372619111111111],
        [5.102542288888889, 0.927746688888889, 2.047265911111111],
        [5.320558288888889, -1.714537311111111, -1.3184900888888889],
        [6.4033382888888895, -0.991211311111111, 1.0667899111111112],
        [7.080085288888889, -2.067315311111111, 0.4565679111111111],
        [7.8904582888888894, -2.333966311111111, 1.1291909111111111],
        [6.421497288888889, -2.929765311111111, 0.3262879111111111],
        [7.496049288888889, -1.778685311111111, -0.512051088888889],
        [0.8193432888888893, 2.478773688888889, 1.6568509111111112],
        [1.5523202888888894, 1.0265216888888888, -2.448307088888889],
    ]
    ,
    "charge": 0,
    "spin": 1,
    "size": 0.0

}

LIGANDS = {d["name"]: d for d in (octinoxate_data, octocrylene_data, oxybenzone_data, avobenzone_data)}

def define_fragments():
    frags = []

    frags.append(
        {
            "num_atoms": 19,
            "name": "arginine",
            "atoms": ['N', 'H', 'H', 'C', 'N', 'H', 'H', 'N', 'H', 'C', 'H', 'H', 'C', 'H', 'H', 'C', 'H', 'H', 'H'],
            "coords": [
                [-2.149366052631579, 1.280494894736842, 5.8421052631544205e-06],
                [-1.3771460526315789, 1.920837894736842, -0.00012415789473684558],
                [-3.074224052631579, 1.669389894736842, 0.00018684210526315442],
                [-1.9501300526315792, -0.03491610526315792, -2.5157894736845577e-05],
                [-3.000153052631579, -0.8577391052631579, -1.0157894736845579e-05],
                [-2.8825910526315788, -1.854114105263158, 0.00026584210526315444],
                [-3.940025052631579, -0.5069571052631578, 0.00010984210526315442],
                [-0.719979052631579, -0.519462105263158, -9.415789473684558e-05],
                [-0.605082052631579, -1.5188031052631579, -0.00027215789473684557],
                [0.501152947368421, 0.2875508947368421, -8.515789473684558e-05],
                [0.511472947368421, 0.9243478947368421, -0.8887071578947369],
                [0.5113809473684209, 0.924442894736842, 0.8884678421052632],
                [1.7269709473684212, -0.611826105263158, 2.684210526315442e-05],
                [1.6952549473684209, -1.2578831052631578, -0.8806161578947368],
                [1.695172947368421, -1.2577721052631579, 0.8807468421052632],
                [3.014574947368421, 0.20012289473684208, 2.9842105263154425e-05],
                [3.8796049473684207, -0.4595711052631579, 0.00011184210526315441],
                [3.081593947368421, 0.8358708947368421, -0.8833471578947368],
                [3.081516947368421, 0.835985894736842, 0.8833298421052632]
            ],
            "charge": 1,
            "spin": 1,
            "size": 0.0

        }
    )

    frags.append(
        {
            "num_atoms": 17,
            "name": "lysine",
            "atoms": ['N', 'H', 'H', 'C', 'H', 'H', 'C', 'H', 'H', 'C', 'H', 'H', 'C', 'H', 'H', 'H', 'H'],
            "coords": [
                [-0.1042588335294119, -2.00525728, 1.473917954705882],
                [0.22287441647058825, -2.95022911, 1.4704449947058822],
                [0.27274188647058795, -1.5249122800000001, 2.2658391247058822],
                [0.328762786470588, -1.3322867200000001, 0.2408312147058822],
                [-0.07462797352941175, -1.84625587, -0.6065244452941179],
                [1.397378736470588, -1.3395881200000002, 0.18691787470588217],
                [-0.17502241352941184, 0.1229699099999999, 0.2461795647058822],
                [-1.2436383635294117, 0.13027130999999992, 0.30009290470588224],
                [0.22836834647058835, 0.63693906, 1.093535224705882],
                [0.27861927647058815, 0.8279866899999999, -1.0456255952941178],
                [-0.12477148352941203, 0.31401754000000004, -1.8929812452941177],
                [1.347235226470588, 0.8206852799999997, -1.0995389252941177],
                [-0.22516592352941167, 2.28324332, -1.0402772352941176],
                [0.17822483647058807, 2.79721246, -0.19292158529411796],
                [0.09002667647058793, 2.77309263, -1.9378301752941178],
                [-1.293781873529412, 2.2905447199999998, -0.9863639052941178],
                [-1.102965323529412, -1.9984335400000002, 1.524304254705882]
            ],
            "charge":1,
            "spin": 1,
            "size": 0.0

        }
    )

    frags.append(
        {
            "num_atoms": 7,
            "name": "aspartic",
            "atoms": ['O', 'C', 'C', 'H', 'H', 'H', 'O'],
            "coords": [
                [1.6558742642857145, 0.7319832657142856, -0.34302586285714287],
                [0.4477709842857145, 0.7119481857142856, 0.008621177142857157],
                [-0.34072376571428564, -0.6105680342857143, -0.020134372857142844],
                [-0.24345327571428554, -1.1044111942857144, 0.9240887971428572],
                [-1.3735099757142855, -0.4059838542857144, -0.21091682285714283],
                [0.046941714285714475, -1.2402001942857144, -0.7935545728571428],
                [-0.19289994571428548, 1.9172318257142857, 0.43492165714285713]
            ],
            "charge": -1,
            "spin": 1,
            "size": 0.0
        }
    )

    # propionate: the glutamic side chain capped with a methyl where CB attaches.
        # Replaces an entry that was acetate, i.e. identical to the aspartic entry.
    frags.append(
        {
            "num_atoms": 10,
            "name": "glutamic",
            "atoms": ['C', 'C', 'C', 'O', 'O', 'H', 'H', 'H', 'H', 'H'],
            "coords": [
                [-1.069447, 0.497533, -0.047868],
                [0.041223, -0.460676, 0.336367],
                [1.403861, -0.173827, -0.300315],
                [1.468726, 0.820552, -1.073714],
                [2.312017, -0.987662, 0.036848],
                [-0.822755, 1.523859, 0.243049],
                [-1.244853, 0.484595, -1.128561],
                [-2.00274, 0.218211, 0.450984],
                [0.167427, -0.443155, 1.425436],
                [-0.253458, -1.479429, 0.057774]
            ],
            "charge": -1,
            "spin": 1,
            "size": 0.0
        }
    )

    frags.append(
        {
            "num_atoms": 14,
            "name": "isoleucine",
            "atoms": ['C', 'H', 'C', 'C', 'H', 'H', 'H', 'H', 'H', 'C', 'H', 'H', 'H', 'H'],
            "coords": [
                [-0.248106, 0.722549, 0.6713884285714286],
                [-1.342172, 0.724819, 0.6832444285714285],
                [0.248106, 1.548297, -0.5100675714285714],
                [0.248106, -0.722549, 0.6713884285714286],
                [1.339315, 1.545443, -0.5559105714285714],
                [-0.12406199999999999, 1.160183, -1.4588485714285715],
                [-0.07900799999999998, 2.585336, -0.4300755714285714],
                [1.342172, -0.724819, 0.6832444285714285],
                [-0.06799399999999999, -1.203132, 1.6002694285714285],
                [-0.248106, -1.548297, -0.5100675714285714],
                [0.12406200000000002, -1.160183, -1.4588485714285715],
                [0.07900800000000001, -2.585336, -0.4300755714285714],
                [-1.339315, -1.545443, -0.5559105714285714],
                [0.06799400000000001, 1.203132, 1.6002694285714285]
            ],
            "charge": 0,
            "spin": 1,
            "size": 0.0
        }
    )

    frags.append(
        {
            "num_atoms": 14,
            "name": "leucine",
            "atoms": ['C', 'H', 'H', 'C', 'H', 'C', 'C', 'H', 'H', 'H', 'H', 'H', 'H', 'H'],
            "coords": [
                [-7.93016446160826e-18, 1.4499611428571428, -0.06375257142857141],
                [-7.93016446160826e-18, 1.4968431428571427, -1.1564665714285716],
                [0.883012, 1.9845861428571427, 0.2916454285714286],
                [-7.93016446160826e-18, 1.4285714282953016e-07, 0.4077454285714286],
                [-7.93016446160826e-18, 1.4285714282953016e-07, 1.5030394285714286],
                [-1.255703, -0.7249808571428572, -0.06375257142857141],
                [1.255703, -0.7249808571428572, -0.06375257142857141],
                [-1.296304, -0.7484218571428572, -1.1564665714285716],
                [-1.277196, -1.7570038571428572, 0.2916454285714286],
                [-2.160208, -0.22758185714285717, 0.2916454285714286],
                [2.160208, -0.22758185714285717, 0.2916454285714286],
                [1.277196, -1.7570038571428572, 0.2916454285714286],
                [1.296304, -0.7484218571428572, -1.1564665714285716],
                [-0.883012, 1.9845861428571427, 0.2916454285714286]
            ]
            ,
            "charge": 0,
            "spin": 1,
            "size": 0.0

        }
    )

    frags.append(
        {
            "num_atoms": 6,
            "name": "serine",
            "atoms": ['O', 'H', 'C', 'H', 'H', 'H'],
            "coords": [
                    [-0.1389911666666667, -1.0804205, 0.0],
                    [0.7654168333333333, -1.3877454999999999, 0.0],
                    [-0.1389911666666667, 0.33025350000000003, 0.0],
                    [-1.1797341666666665, 0.6498875, 0.0],
                    [0.3461498333333333, 0.7440125, 0.890351],
                    [0.3461498333333333, 0.7440125, -0.890351]
            ]
            ,
            "charge": 0,
            "spin": 1,
            "size": 0.0
        }
    )

    frags.append(
        {
            "num_atoms":19,
            "name": "tryptophan",
            "atoms": ['C', 'C', 'C', 'C', 'C', 'C', 'H', 'H', 'H', 'H', 'C', 'C', 'H', 'N', 'H', 'C', 'H', 'H', 'H'],
            "coords": [
                    [0.799817, -1.3618299473684212, -5.268421052631055e-05],
                    [-0.040227999999999986, -0.244916947368421, 1.3157894736894564e-06],
                    [0.532212, 1.042519052631579, -4.268421052631054e-05],
                    [1.910519, 1.2401040526315792, -8.868421052631054e-05],
                    [2.714339, 0.11930905263157897, 3.3315789473689454e-05],
                    [2.165274, -1.171611947368421, 6.431578947368946e-05],
                    [0.38329199999999997, -2.361199947368421, -1.7684210526310542e-05],
                    [2.338177, 2.234374052631579, -6.968421052631054e-05],
                    [3.789782, 0.23765805263157896, 0.00011731578947368945],
                    [2.827406, -2.027096947368421, 0.00015931578947368945],
                    [-1.4673239999999999, -0.08076594736842102, 8.431578947368946e-05],
                    [-1.692607, 1.2605200526315792, 6.931578947368945e-05],
                    [-2.629076, 1.7942460526315789, 0.00018831578947368945],
                    [-0.49810599999999994, 1.9450620526315792, 3.5315789473689455e-05],
                    [-0.40103599999999995, 2.940852052631579, -0.00019368421052631053],
                    [-2.482686, -1.173145947368421, -5.068421052631055e-05],
                    [-2.377944, -1.8117019473684208, -0.8797486842105263],
                    [-2.376954, -1.812779947368421, 0.8787403157894736],
                    [-3.494857, -0.769594947368421, 0.0007713157894736895]
            ]
            ,
            "charge": 0,
            "spin": 1,
            "size": 0.0
        }
    )

    # p-cresol: the tyrosine side chain capped with a methyl where CB attaches.
        # Replaces an entry that was a C7H7 radical (odd electron count, declared spin 1)
        # plus a detached water, with no phenol oxygen at all.
    frags.append(
        {
            "num_atoms": 16,
            "name": "tyrosine",
            "atoms": ['C', 'C', 'C', 'C', 'C', 'O', 'C', 'C', 'H', 'H', 'H', 'H', 'H', 'H', 'H', 'H'],
            "coords": [
                [2.469401, -0.129617, 0.040839],
                [0.974092, -0.022437, -0.012353],
                [0.357391, 1.229514, -0.134511],
                [-1.0345, 1.3319, -0.152288],
                [-1.810897, 0.185665, -0.037441],
                [-3.166582, 0.331008, -0.058972],
                [-1.218073, -1.065138, 0.099114],
                [0.175175, -1.166706, 0.116802],
                [2.939102, 0.661753, -0.552729],
                [2.807118, -1.086931, -0.369884],
                [2.813698, -0.047343, 1.076304],
                [0.956664, 2.133146, -0.217502],
                [-1.51051, 2.302944, -0.252452],
                [-3.57112, -0.547308, 0.02822],
                [-1.816674, -1.965004, 0.195508],
                [0.635717, -2.145445, 0.231344]
            ],
            "charge": 0,
            "spin": 1,
            "size": 0.0
        }
    )

    frags.append(
        {
            "num_atoms": 15,
            "name": "phenylalanine",
            "atoms": ['C', 'C', 'C', 'C', 'C', 'C', 'H', 'H', 'H', 'H', 'H', 'C', 'H', 'H', 'H'],
            "coords": [
                    [-1.041578616666666, -1.803521892666667, 0.0006557966666666665],
                    [0.3535813833333341, -1.803521892666667, 0.0006557966666666665],
                    [1.0511193833333343, -0.5957708926666669, 0.0006557966666666665],
                    [0.3534653833333343, 0.6127381073333331, -0.0005432033333333322],
                    [-1.0413596166666657, 0.6126601073333332, -0.0010222033333333325],
                    [-1.7389606166666662, -0.5955458926666668, -2.6203333333332107e-05],
                    [-1.591337616666666, -2.7558388926666666, 0.0011057966666666655],
                    [0.903089383333334, -2.7560348926666665, 0.0019707966666666667],
                    [2.150799383333334, -0.5956908926666669, 0.0012897966666666656],
                    [-1.5914816166666657, 1.564941107333333, -0.001975203333333335],
                    [-2.8385646166666656, -0.595362892666667, -0.00020620333333333518],
                    [1.1239694533333342, 1.946126077333333, -0.0006258333333333324],
                    [1.322942303333334, 2.243143987333333, 1.0078830466666666],
                    [2.048388123333334, 1.823846937333333, -0.5254095333333333],
                    [0.535927903333334, 2.6978318173333333, -0.4844084433333333]
            ],
            "charge": 0,
            "spin": 1,
            "size": 0.0
        }
    )

    return frags

ONE_LETTER = {
    'tyrosine': 'Y', 'phenylalanine': 'F', 'alanine': 'A', 'threonine': 'T',
    'cysteine': 'C', 'histidine': 'H', 'serine': 'S', 'leucine': 'L',
    'lysine': 'K', 'aspartic': 'D', 'glutamic': 'E', 'proline': 'P',
    'glutamine': 'Q', 'glycine': 'G', 'valine': 'V', 'arginine': 'R',
    'isoleucine': 'I', 'tryptophan': 'W', 'methionine': 'M', 'cystine': 'C',
}

def load_ligand(spec: str, charge=None, spin=None, name=None):
    '''
        Builds a ligand dictionary from a built-in ligand name, an XYZ file or a SMILES string.
        XYZ/SMILES coordinates are centred on their centroid, like the built-in ligands.

        Args:
            spec: built-in name, path to an .xyz file, or a SMILES string
            charge: net charge (default: 0 for XYZ, RDKit formal charge for SMILES)
            spin: spin multiplicity (default: 1 for XYZ, radicals + 1 for SMILES)
            name: ligand name used in output files (default: file stem / "ligand")
        Returns:
            ligand dictionary
    '''
    if spec in LIGANDS:
        ligand = copy.deepcopy(LIGANDS[spec])
        if charge is not None:
            ligand["charge"] = charge
        if spin is not None:
            ligand["spin"] = spin
        return ligand

    if os.path.isfile(spec):
        atoms = ase.io.read(spec)
        default_name = os.path.splitext(os.path.basename(spec))[0]
        default_charge, default_spin = 0, 1
    else:
        atoms, default_charge, default_spin = smiles_to_atoms(spec)
        default_name = "ligand"

    coords = atoms.get_positions()
    coords -= coords.mean(axis=0)
    return {
        "num_atoms": len(atoms),
        "name": name or default_name,
        "atoms": atoms.get_chemical_symbols(),
        "coords": coords.tolist(),
        "charge": default_charge if charge is None else charge,
        "spin": default_spin if spin is None else spin,
        "size": 0.0,
    }


def smiles_to_atoms(smiles: str, n_conformers: int = 5, seed: int = 42):
    '''
        SMILES -> add Hs -> ETKDGv3 conformers -> MMFF (UFF fallback) minimisation
        -> lowest-energy conformer. Same recipe as mace_calc.smiles_to_atoms.

        Returns:
            atoms: ASE Atoms of the lowest-energy conformer
            charge: RDKit formal charge
            spin: multiplicity from radical electron count
    '''
    from rdkit import Chem
    from rdkit.Chem import AllChem, Descriptors

    mol = Chem.MolFromSmiles(smiles)
    if mol is None:
        raise ValueError(f"not a built-in ligand, an existing file, or a SMILES RDKit can parse: {smiles!r}")
    mol = Chem.AddHs(mol)

    params = AllChem.ETKDGv3()
    params.randomSeed = seed
    params.pruneRmsThresh = 0.5
    cids = AllChem.EmbedMultipleConfs(mol, numConfs=n_conformers, params=params)
    if not cids:
        raise RuntimeError(f"RDKit failed to embed any conformers for {smiles!r}")

    props = AllChem.MMFFGetMoleculeProperties(mol)
    energies = []
    for cid in cids:
        if props is None:
            ff = AllChem.UFFGetMoleculeForceField(mol, confId=cid)
        else:
            ff = AllChem.MMFFGetMoleculeForceField(mol, props, confId=cid)
        ff.Minimize(maxIts=500)
        energies.append((ff.CalcEnergy(), cid))
    best_cid = min(energies)[1]

    atoms = Atoms([a.GetSymbol() for a in mol.GetAtoms()],
                  positions=mol.GetConformer(best_cid).GetPositions())
    charge = Chem.GetFormalCharge(mol)
    spin = Descriptors.NumRadicalElectrons(mol) + 1
    return atoms, charge, spin


def get_binding_site_dims(ligand: dict, vdw_distance: float):
    '''
        Uses the array of atom positions to find the maximum and minimum dimensions

        Args:
            ligand: ligand dictionary
            vdw_distance: extra distance to add to binding site extent
        Returns:
            max_values: np.array of maximum values
            min_values: np.array of minimum values
    '''
    coords = np.array(ligand["coords"])
    max_values = coords.max(axis=0) + vdw_distance
    min_values = coords.min(axis=0) - vdw_distance

    print("Binding site dimensions after augmentation:")
    for dim, maxes, mins in zip("xyz", max_values, min_values):
        print(f"  {dim} - Max: {maxes:10.3f}, Min: {mins:10.3f}")
    print(f"  Volume is {np.prod(max_values - min_values):.1f} A^3")

    return max_values, min_values


def get_frag_coordinates(new_origin, which_frag: dict):
    """
        Given an origin set of coordinates for a new fragment, this function
        reads in the standard fragment coordinates, rotates them randomly
        in each direction, and then translates them to the new origin.

        Args:
            new_origin: the location of the new fragment
            which_frag: a dictionary for the fragment in question.
    """
    theta_x = random.uniform(0, 2 * np.pi)
    theta_y = random.uniform(0, 2 * np.pi)
    theta_z = random.uniform(0, 2 * np.pi)
    x_rotation_matrix = np.array([[1.0, 0.0, 0.0], [0.0, np.cos(theta_x), -np.sin(theta_x)], [0.0, np.sin(theta_x), np.cos(theta_x)]])
    y_rotation_matrix = np.array([[np.cos(theta_y), 0.0, -np.sin(theta_y)], [0.0, 1.0, 0.0], [np.sin(theta_y), 0.0, np.cos(theta_y)]])
    z_rotation_matrix = np.array([[np.cos(theta_z), -np.sin(theta_z), 0.0], [np.sin(theta_z), np.cos(theta_z), 0.0], [0.0, 0.0, 1.0]])
    atoms_list = []
    for vec in which_frag['coords']:
        rot_temp = np.matmul(x_rotation_matrix, np.array(vec))
        rot_temp = np.matmul(y_rotation_matrix, rot_temp)
        rot_temp = np.matmul(z_rotation_matrix, rot_temp)
        atoms_list.append(rot_temp + new_origin)
    return np.array(atoms_list)


def calc_frag_energy(new_molecule, frag: dict, atom_symbols: list, calculator, bs_charge, bs_spin, ligand_charge, ligand_spin, opt_flag):
    """
        Calculates the interaction energy between the binding site and a fragment,
        optionally relaxing the fragment (binding site atoms fixed) first.

        Args:
            new_molecule: array of coordinates of the binding site followed by the fragment
            frag: the dictionary for the fragment in question
            atom_symbols: list of binding site atom symbols
            calculator: ASE calculator
            bs_charge, bs_spin: binding site charge and multiplicity
            ligand_charge, ligand_spin: fragment charge and multiplicity
            opt_flag: run a BFGS relaxation of the fragment before scoring
        Returns:
            ie: interaction energy (kcal/mol)
            opt_molecule: np.array of (possibly relaxed) coordinates
    """
    atoms_tot = Atoms(atom_symbols + frag['atoms'], positions=new_molecule)
    total_spin = bs_spin + ligand_spin - 1
    total_charge = bs_charge + ligand_charge
    atoms_tot.info.update({'spin': total_spin, 'charge': total_charge})
    atoms_tot.calc = calculator

    bs_length = len(new_molecule) - frag['num_atoms']
    atoms_tot.set_constraint(FixAtoms(indices=list(range(bs_length))))
    if opt_flag:
        # logfile='-' not None: this relaxation runs once per pose, and a sweep killed part way
        # through used to leave no trace of which poses had been done or how far each got.
        # steps caps a pose that will not settle, so one bad geometry cannot stall a whole sweep.
        BFGS(atoms_tot, logfile='-').run(fmax=0.05, steps=200)

    atoms_bs = atoms_tot[:bs_length]
    atoms_bs.info.update({'spin': bs_spin, 'charge': bs_charge})
    atoms_l = atoms_tot[bs_length:]
    atoms_l.info.update({'spin': ligand_spin, 'charge': ligand_charge})
    results = []
    for atoms in [atoms_tot, atoms_bs, atoms_l]:
        atoms.calc = calculator
        results.append(atoms.get_potential_energy())
    ie = 23.06035 * (results[0] - results[1] - results[2])

    return ie, atoms_tot.get_positions()


def grow_fragments(ligand: dict, frags: list, number_tries: int, calculator, max_values, min_values,
                   energy_cutoff: float = 500.0, pose_dir: str = None):
    """
        Accepts the binding site and fragment dictionaries and tries to add the
        fragments to the binding site. Placements with an unrelaxed interaction
        energy below energy_cutoff are relaxed with BFGS and kept. If pose_dir is
        given, each kept pose is written there as soon as it is found, so an
        interrupted run keeps its work.

        Args:
            ligand: binding site (ligand) dictionary
            frags: list of fragment dictionaries
            number_tries: how many times to attempt fragment placement
            calculator: ASE calculator
            max_values: np.array of maximum values
            min_values: np.array of minimum values
            energy_cutoff: keep placements with IE below this (kcal/mol)
            pose_dir: directory for <ligand>_w_<fragment><index>.xyz files (IE in the comment line)
        Returns:
            new_molecules: per fragment, list of coordinate arrays (binding site + fragment)
            ies: per fragment, list of interaction energies (kcal/mol)
    """
    centers = [(max_values[i] + min_values[i]) / 2 for i in range(3)]
    new_molecules = []
    ies = []
    for frag in frags:
        print(f"Fragment: {frag['name']}", flush=True)
        new_sheet = []
        ie_sheet = []
        for _ in range(number_tries):
            new_mol_origin = np.array([np.random.normal(loc=centers[i], scale=0.5 * max_values[i]) for i in range(3)])
            new_vec = get_frag_coordinates(new_mol_origin, frag)
            single_frag = np.append(ligand["coords"], new_vec, axis=0)

            args = (frag, ligand['atoms'], calculator, ligand['charge'], ligand['spin'], frag['charge'], frag['spin'])
            ie, single_frag = calc_frag_energy(single_frag, *args, opt_flag=False)
            if ie < energy_cutoff:
                ie, single_frag = calc_frag_energy(single_frag, *args, opt_flag=True)
                print(f"  adding {frag['name']} pose {len(new_sheet)}: IE = {ie:.2f} kcal/mol", flush=True)
                if pose_dir:
                    write_xyz(os.path.join(pose_dir, f"{ligand['name']}_w_{frag['name']}{len(new_sheet)}.xyz"),
                              ligand["atoms"] + frag["atoms"], single_frag, f"IE = {ie:.3f} kcal/mol")
                new_sheet.append(single_frag)
                ie_sheet.append(ie)
        print(f"Added {len(new_sheet)} {frag['name']} fragments", flush=True)
        new_molecules.append(new_sheet)
        ies.append(ie_sheet)
    return new_molecules, ies


def write_xyz(path, symbols, coords, comment=""):
    with open(path, "w") as f:
        f.write(f"{len(symbols)}\n{comment}\n")
        for symbol, row in zip(symbols, coords):
            f.write(f"{symbol}" + "".join(f"    {c}" for c in row) + "\n")


def distance_too_short(frag_coords1, frag_coords2, threshold=1.4):
    for atom1 in frag_coords1:
        for atom2 in frag_coords2:
            if np.linalg.norm(np.array(atom1) - np.array(atom2)) < threshold:
                return True
    return False


def select_poses(ies: list, frags: list, ligand: dict, new_molecules: list, threshold: float = 1.4,
                 order=None, copies: int = 1, verbose: bool = True):
    '''
      Greedy selection: walk the fragments in `order`, each taking its lowest-IE pose
      that does not come within `threshold` of any pose already selected. The walk is
      repeated `copies` times, so a fragment can be placed more than once (each copy
      takes a different pose).

      Args:
        ies: per fragment, list of interaction energies
        frags: list of fragment dictionaries
        ligand: binding site dictionary
        new_molecules: per fragment, list of coordinate arrays
        threshold: minimum distance between atoms of selected fragments
        order: fragment indices, in the order they get to pick (default: library order)
        copies: how many times each fragment may be placed
      Returns:
        selected: list of (frag_idx, pose_idx) in the order they were placed
    '''
    if order is None:
        order = list(range(len(frags)))
    selected = []
    chosen_coords = []
    used = {}
    for _ in range(copies):
        for idx in order:
            taken = used.setdefault(idx, set())
            for pose_idx in np.argsort(ies[idx]):
                pose_idx = int(pose_idx)
                if pose_idx in taken:
                    continue
                coords = new_molecules[idx][pose_idx][ligand["num_atoms"]:]
                if any(distance_too_short(prev, coords, threshold) for prev in chosen_coords):
                    continue
                selected.append((idx, pose_idx))
                chosen_coords.append(coords)
                taken.add(pose_idx)
                if verbose:
                    print(f"placed {frags[idx]['name']}: IE {ies[idx][pose_idx]:.3f} at pose {pose_idx}")
                break
    return selected


def combine_poses(ligand: dict, frags: list, new_molecules: list, selected: list, path: str):
    '''
      Combines the ligand with every selected pose and writes it to `path`.

      Args:
        selected: list of (frag_idx, pose_idx) from select_poses
      Returns:
        combined_poses: combined coordinates
        combined_atoms: combined atom symbols
        all_centers: centroid of each placed fragment
        all_names: name of each placed fragment
    '''
    combined_poses = list(ligand["coords"])
    combined_atoms = list(ligand["atoms"])
    all_centers = []
    all_names = []
    for idx_frag, pose in selected:
        frag = frags[idx_frag]
        just_frag_array = new_molecules[idx_frag][pose][ligand["num_atoms"]:]
        all_centers.append(np.mean(just_frag_array, axis=0))
        all_names.append(frag["name"])
        combined_poses.extend(just_frag_array)
        combined_atoms.extend(frag["atoms"])

    write_xyz(path, combined_atoms, combined_poses, "placed: " + " ".join(all_names))
    return combined_poses, combined_atoms, all_centers, all_names


def build_sequence(all_centers: list, all_names: list, linker_spacing: float = 3.8,
                   cyclic: bool = False, linker_slack: int = 0, verbose: bool = True):
    '''
      Orders the placed fragments into a chain, starting at the first one and always
      stepping to the nearest unvisited fragment centre. Gaps longer than one residue
      spacing are filled with glycine linkers: round(distance / 3.8) - 1 of them.

      round(distance / 3.8) - 1 is the taut minimum, which leaves the backbone no slack to
      place a side chain where the fragment was. linker_slack adds that many extra glycines to
      every gap, trading chain length for the flexibility to reach the designed positions.

      With cyclic=True the chain is closed: the gap from the last fragment back to the
      first is measured too, and its linkers are appended so the ends can be joined.

      Returns:
        sequence: one-letter peptide sequence
        steps: list of (name_from, name_to, distance, n_linkers); the last entry is the
               ring closure when cyclic=True
    '''
    if not all_centers:
        return "", []
    remaining = list(range(1, len(all_centers)))
    current = 0
    sequence = ONE_LETTER[all_names[0]]
    steps = []

    def add_step(frm, to, distance):
        nonlocal sequence
        n_linkers = max(round(distance / linker_spacing) - 1, 0) + linker_slack
        if verbose:
            print(f"distance between {all_names[frm]}, {all_names[to]} is {distance:.2f}"
                  + (f" -> add {n_linkers} linkers" if n_linkers else ""))
        steps.append((all_names[frm], all_names[to], distance, n_linkers))
        return n_linkers

    while remaining:
        distances = [np.linalg.norm(np.array(all_centers[i]) - np.array(all_centers[current])) for i in remaining]
        nearest = remaining.pop(int(np.argmin(distances)))
        n_linkers = add_step(current, nearest, float(min(distances)))
        sequence += "G" * n_linkers + ONE_LETTER[all_names[nearest]]
        current = nearest

    if cyclic and len(all_centers) > 2:
        closing = float(np.linalg.norm(np.array(all_centers[0]) - np.array(all_centers[current])))
        n_linkers = add_step(current, 0, closing)
        sequence += "G" * n_linkers
        if verbose:
            print(f"ring closure needs {n_linkers} linkers")
    if verbose:
        print(f"cyclo-{sequence}" if cyclic else sequence)
    return sequence, steps


def load_poses(outdir: str, frags: list, ligand: dict):
    """
        Reads poses/*.xyz and energies.csv from an earlier run back into the
        (new_molecules, ies) form, so combinations can be re-run without re-sampling.

        Returns:
            new_molecules: per fragment, list of coordinate arrays
            ies: per fragment, list of interaction energies
    """
    by_name = {frag["name"]: i for i, frag in enumerate(frags)}
    new_molecules = [[] for _ in frags]
    ies = [[] for _ in frags]
    with open(os.path.join(outdir, "energies.csv")) as f:
        rows = sorted(csv.DictReader(f), key=lambda r: (r["fragment"], int(r["pose"])))
    for row in rows:
        idx = by_name[row["fragment"]]
        path = os.path.join(outdir, "poses", f"{ligand['name']}_w_{row['fragment']}{row['pose']}.xyz")
        new_molecules[idx].append(ase.io.read(path).get_positions())
        ies[idx].append(float(row["ie_kcal_mol"]))
    print(f"loaded {sum(len(m) for m in new_molecules)} poses from {outdir}")
    return new_molecules, ies


def iterate_combinations(ligand: dict, frags: list, new_molecules: list, ies: list, outdir: str,
                         threshold: float = 1.4, copies: int = 1, starts=None, cyclic: bool = False,
                         linker_slack: int = 0):
    """
        Runs the selection once per starting fragment: the fragment order is rotated
        so each one in turn gets first pick, which gives a different set of poses and
        so a different peptide. Every distinct sequence is saved; when two starts give
        the same sequence, the one with the lower total IE is kept.

        Args:
            threshold: minimum distance between atoms of selected fragments
            copies: how many times each fragment may be placed
            starts: fragment indices to start from (default: every fragment with poses)
        Returns:
            results: one dict per unique sequence, sorted by total IE
    """
    seq_dir = os.path.join(outdir, "sequences")
    os.makedirs(seq_dir, exist_ok=True)
    if starts is None:
        starts = [i for i in range(len(frags)) if ies[i]]

    unique = {}
    for start in starts:
        order = [(start + k) % len(frags) for k in range(len(frags))]
        selected = select_poses(ies, frags, ligand, new_molecules, threshold, order, copies, verbose=False)
        if not selected:
            continue
        _, _, centers, names = combine_poses(ligand, frags, new_molecules, selected,
                                             os.path.join(seq_dir, "_scratch.xyz"))
        sequence, steps = build_sequence(centers, names, cyclic=cyclic,
                                         linker_slack=linker_slack, verbose=False)
        total_ie = sum(ies[i][p] for i, p in selected)
        record = {"sequence": sequence, "start": frags[start]["name"], "n_fragments": len(selected),
                  "total_ie": total_ie, "selected": selected, "steps": steps}
        print(f"start {frags[start]['name']:<14} -> {('cyclo-' + sequence) if cyclic else sequence:<26} {len(selected):>2} fragments, total IE {total_ie:8.2f}")
        if sequence not in unique or total_ie < unique[sequence]["total_ie"]:
            unique[sequence] = record

    scratch = os.path.join(seq_dir, "_scratch.xyz")
    if os.path.exists(scratch):
        os.remove(scratch)

    results = sorted(unique.values(), key=lambda r: r["total_ie"])
    for record in results:
        combine_poses(ligand, frags, new_molecules, record["selected"],
                      os.path.join(seq_dir, ("cyclo_" if cyclic else "") + f"{record['sequence']}.xyz"))
    # merge with any sequences from earlier passes, so the csv matches the folder
    table = {}
    csv_path = os.path.join(outdir, "sequences.csv")
    if os.path.exists(csv_path):
        with open(csv_path) as f:
            for row in csv.DictReader(f):
                table[row["sequence"]] = row
    for record in results:
        row = {"sequence": ("cyclo-" if cyclic else "") + record["sequence"], "start_residue": record["start"],
               "n_fragments": str(record["n_fragments"]), "copies": str(copies),
               "total_ie_kcal_mol": f"{record['total_ie']:.4f}"}
        old_row = table.get(row["sequence"])
        if old_row is None or float(row["total_ie_kcal_mol"]) < float(old_row["total_ie_kcal_mol"]):
            table[row["sequence"]] = row
    fields = ["sequence", "start_residue", "n_fragments", "copies", "total_ie_kcal_mol"]
    with open(csv_path, "w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=fields)
        writer.writeheader()
        for row in sorted(table.values(), key=lambda r: float(r["total_ie_kcal_mol"])):
            writer.writerow({k: row.get(k, "") for k in fields})
    print(f"\n{len(results)} sequences from this pass; {len(table)} unique in total, saved to {seq_dir}/")
    return results


# ---------------------------------------------------------------- notebook viewers
def view_xyz(path: str, n_ligand_atoms: int, width=800, height=400):
    '''
      Displays an XYZ file written by peptidebuilder: ligand atoms as default
      ball-and-stick, fragment atoms in green.
    '''
    import py3Dmol  # optional, only needed in notebooks
    with open(path) as f:
        mol_data = f.read()
    viewer = py3Dmol.view(width=width, height=height)
    viewer.addModel(mol_data, "xyz")
    viewer.setStyle({"stick": {}, "sphere": {"radius": 0.5}})
    viewer.setStyle({"serial": f"{n_ligand_atoms}-100000"}, {"stick": {}, "sphere": {"radius": 0.5}, "color": "green"})
    viewer.zoomTo()
    viewer.show()


def view_frag_pose(outdir: str, ligand: dict, frag_name: str, pose_idx: int):
    view_xyz(os.path.join(outdir, "poses", f"{ligand['name']}_w_{frag_name}{pose_idx}.xyz"), ligand["num_atoms"])


def view_combined_poses(outdir: str, ligand: dict):
    view_xyz(os.path.join(outdir, "combined.xyz"), ligand["num_atoms"])


# ---------------------------------------------------------------- command line
def select_fragments(spec):
    '''Fragments by comma-separated names or one-letter codes; all of them if spec is None.'''
    frags = define_fragments()
    if spec is None:
        return frags
    by_key = {}
    for frag in frags:
        by_key[frag["name"]] = frag
        by_key[ONE_LETTER[frag["name"]]] = frag
    chosen = []
    for key in spec.split(","):
        key = key.strip()
        key = key.upper() if len(key) == 1 else key.lower()
        if key not in by_key:
            sys.exit(f"unknown fragment {key!r}; run `peptidebuilder list` to see the library")
        chosen.append(by_key[key])
    return chosen


def cmd_list(args):
    print("Built-in ligands:")
    for name, lig in LIGANDS.items():
        print(f"  {name:<14} {lig['num_atoms']:>3} atoms  charge {lig['charge']}  spin {lig['spin']}")
    print("Fragments:")
    for frag in define_fragments():
        print(f"  {ONE_LETTER[frag['name']]}  {frag['name']:<14} {frag['num_atoms']:>3} atoms  charge {frag['charge']}")


def cmd_run(args):
    # heavy imports here so `list` and `--help` stay fast
    import torch
    from fairchem.core import FAIRChemCalculator, pretrained_mlip

    if args.seed is not None:
        random.seed(args.seed)
        np.random.seed(args.seed)

    ligand = load_ligand(args.ligand, args.charge, args.spin, args.name)
    frags = select_fragments(args.fragments)
    outdir = args.outdir or os.path.join("runs", ligand["name"])
    pose_dir = os.path.join(outdir, "poses")
    os.makedirs(pose_dir, exist_ok=True)
    if any(f.endswith(".xyz") for f in os.listdir(pose_dir)):
        sys.exit(f"{pose_dir} already has pose files from an earlier run; choose another --outdir")
    write_xyz(os.path.join(outdir, "ligand.xyz"), ligand["atoms"], ligand["coords"], ligand["name"])
    print(f"Ligand: {ligand['name']} ({ligand['num_atoms']} atoms, charge {ligand['charge']}, spin {ligand['spin']})")
    print(f"Fragments: {', '.join(f['name'] for f in frags)}")

    device = args.device
    if device == "auto":
        device = "cuda" if torch.cuda.is_available() else "cpu"
    print(f"Loading {args.model} on {device}", flush=True)
    if os.path.isfile(args.model):
        predictor = pretrained_mlip.load_predict_unit(args.model, device=device)
    else:
        predictor = pretrained_mlip.get_predict_unit(args.model, device=device)
    calculator = FAIRChemCalculator(predictor, task_name="omol")

    max_values, min_values = get_binding_site_dims(ligand, args.box_pad)
    start = time.time()
    new_molecules, ies = grow_fragments(ligand, frags, args.tries, calculator, max_values, min_values,
                                    energy_cutoff=args.energy_cutoff, pose_dir=pose_dir)
    print(f"Sampling took {time.time() - start:.0f} s")

    with open(os.path.join(outdir, "energies.csv"), "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["fragment", "pose", "ie_kcal_mol"])
        for frag, frag_ies in zip(frags, ies):
            for j, ie in enumerate(frag_ies):
                writer.writerow([frag["name"], j, f"{ie:.4f}"])

    write_combinations(ligand, frags, new_molecules, ies, outdir, args)


def write_combinations(ligand, frags, new_molecules, ies, outdir, args):
    """Selection, combination and sequences; shared by `run` and `combine`."""
    starts = None if args.all_starts else [0]
    results = iterate_combinations(ligand, frags, new_molecules, ies, outdir,
                                   threshold=args.clash, copies=args.copies, starts=starts,
                                   cyclic=args.cyclic, linker_slack=args.linker_slack)
    if not results:
        print("\nNo fragments placed.")
        return
    best = results[0]
    combine_poses(ligand, frags, new_molecules, best["selected"], os.path.join(outdir, "combined.xyz"))
    with open(os.path.join(outdir, "sequence.txt"), "w") as f:
        f.write(("cyclo-" if args.cyclic else "") + f"{best['sequence']}\n")
        for name_from, name_to, distance, n_linkers in best["steps"]:
            f.write(f"{name_from} -> {name_to}: {distance:.2f} A, {n_linkers} linkers\n")

    print(f"\nBest sequence: {best['sequence']}  (start {best['start']}, total IE {best['total_ie']:.2f} kcal/mol)")
    print(f"Results in {outdir}/")


def cmd_combine(args):
    """Re-run selection on poses already saved by an earlier `run`, without re-sampling."""
    outdir = args.outdir
    lig_atoms = ase.io.read(os.path.join(outdir, "ligand.xyz"))
    with open(os.path.join(outdir, "ligand.xyz")) as f:
        name = f.readlines()[1].strip() or "ligand"
    ligand = {"num_atoms": len(lig_atoms), "name": name, "atoms": lig_atoms.get_chemical_symbols(),
              "coords": lig_atoms.get_positions().tolist(), "charge": 0, "spin": 1, "size": 0.0}
    frags = define_fragments()
    new_molecules, ies = load_poses(outdir, frags, ligand)
    write_combinations(ligand, frags, new_molecules, ies, outdir, args)


def main(argv=None):
    parser = argparse.ArgumentParser(
        prog="peptidebuilder",
        description="Fragment-based peptide design around a small-molecule ligand using the FAIRChem UMA potential.")
    sub = parser.add_subparsers(dest="command", required=True)

    p_list = sub.add_parser("list", help="show built-in ligands and the fragment library")
    p_list.set_defaults(func=cmd_list)

    p_run = sub.add_parser("run", help="grow fragments around a ligand and build a peptide sequence")
    p_run.add_argument("ligand", help="built-in ligand name, path to an .xyz file, or a SMILES string")
    p_run.add_argument("--name", help="ligand name for output files (default: file stem, or 'ligand' for SMILES)")
    p_run.add_argument("--charge", type=int, help="ligand net charge (default: 0 for XYZ, formal charge for SMILES)")
    p_run.add_argument("--spin", type=int, help="ligand spin multiplicity (default: 1 for XYZ, from radicals for SMILES)")
    p_run.add_argument("-f", "--fragments", help="comma-separated fragment names or one-letter codes, e.g. R,K,W (default: all)")
    p_run.add_argument("-n", "--tries", type=int, default=100, help="placement attempts per fragment (default: 100)")
    p_run.add_argument("--box-pad", type=float, default=0.0, help="distance added to the ligand bounding box, A (default: 0)")
    p_run.add_argument("--energy-cutoff", type=float, default=500.0, help="keep placements with unrelaxed IE below this, kcal/mol (default: 500)")
    p_run.add_argument("--clash", type=float, default=1.4, help="min distance between selected fragments, A (default: 1.4)")
    p_run.add_argument("--copies", type=int, default=1, help="how many times each residue may be placed (default: 1)")
    p_run.add_argument("--all-starts", action="store_true", help="repeat the combination with each residue picking first, and save every unique sequence")
    p_run.add_argument("--cyclic", action="store_true", help="close the chain into a ring, adding linkers for the last-to-first gap")
    p_run.add_argument("--linker-slack", type=int, default=0, help="extra glycines per gap beyond the taut minimum, for backbone flexibility (default: 0)")
    p_run.add_argument("--model", default="uma-s-1p2p1", help="FAIRChem model name or checkpoint .pt path (default: uma-s-1p2p1)")
    p_run.add_argument("--device", default="auto", help="cpu, cuda or auto (default: auto)")
    p_run.add_argument("--seed", type=int, help="random seed for reproducible sampling")
    p_run.add_argument("-o", "--outdir", help="output directory (default: runs/<ligand>)")
    p_run.set_defaults(func=cmd_run)

    p_combine = sub.add_parser("combine", help="rebuild sequences from the poses of an earlier run (no sampling)")
    p_combine.add_argument("outdir", help="directory of an earlier run, e.g. runs/octinoxate")
    p_combine.add_argument("--clash", type=float, default=1.4, help="min distance between selected fragments, A (default: 1.4)")
    p_combine.add_argument("--copies", type=int, default=1, help="how many times each residue may be placed (default: 1)")
    p_combine.add_argument("--all-starts", action="store_true", help="repeat the combination with each residue picking first, and save every unique sequence")
    p_combine.add_argument("--cyclic", action="store_true", help="close the chain into a ring, adding linkers for the last-to-first gap")
    p_combine.add_argument("--linker-slack", type=int, default=0, help="extra glycines per gap beyond the taut minimum, for backbone flexibility (default: 0)")
    p_combine.set_defaults(func=cmd_combine)

    args = parser.parse_args(argv)
    args.func(args)


if __name__ == "__main__":
    main()
